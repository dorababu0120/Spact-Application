...
const createTask = asyncHandler(async (req, res) => {
  try {
    ...
    const { title, team, stage, date, priority, assets,links } = req.body;
...
const updateTask = asyncHandler(async (req, res) => {
  const { id } = req.params;
  const { title, date, team, stage, priority, assets,links } = req.body;
...
const getTasks = asyncHandler(async (req, res) => {
  ...
  const tasksWithAttachmentCount = tasks.map(task => {
    const t = task.toObject();
    t.attachmentCount = (t.assets?.length || 0) + (t.links?.length || 0);
    return t;
  });
...
const getTask = asyncHandler(async (req, res) => {
  try {
    ...
    taskObj.attachmentCount = (taskObj.assets?.length || 0) + (taskObj.links?.length || 0);
...
/* ✅ Leave Task Controller */
const leaveTask = asyncHandler(async (req, res) => {
  const { id } = req.params;
  const { userId } = req.user;
  ...
  try {
    ...
    task.team = task.team.filter((memberId) => memberId.toString() !== userId);

    const activity = {
      type: "left",
      activity: `${leavingUser.name} left the task.`,
      by: userId,
    };
    ...
    await Notice.create({
      team: admins.map(admin => admin._id),
      text: `${leavingUser.name} left the task "${task.title}".`,
      task: task._id,
    });
    ...
    console.log(error);
    res.status(500).json({ status: false, message: "Failed to leave task", error });
  }
});
...
export {
  ...
  updateTask,
  updateTaskStage,
  leaveTask, // ✅ export the new controller
};
